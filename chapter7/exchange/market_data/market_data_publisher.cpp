...
namespace Exchange {
  MarketDataPublisher::MarketDataPublisher(MarketUpdateLFQueue *market_updates, const std::string &iface,
                                           const std::string &snapshot_ip, int shapshot_port,
  ...
  auto MarketDataPublisher::run() noexcept -> void {
    ...
    while (run_) {
      for (auto market_update = outgoing_md_updates_->getNextToRead();
           outgoing_md_updates_->size() && market_update; market_update = outgoing_md_udpates_->getNextToRead()) {
        ...
        snapshot_md_udpates_.updateWriteIndex();
