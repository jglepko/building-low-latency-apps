  ...
  private:
    auto updateNextFreeIndex() noexcept {
      ...
      while (!store_[next_free_index_].is_free_) {
        ++next_free_index_;
        if (UNLIKELY(next_free_index_ == store.size())) { // hardware branch predictor should almost always predict this to be false anyways.
          next_free_index_ = 0;
        }
    ...
    // It is better to have one vector of structs with two objects.
    struct ObjectBlock {
      T object_;
      bool is_free_ = true;
    };
