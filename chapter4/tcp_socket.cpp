...
namespace Common {
  ...
  auto TCPSocket::sendAndRecv() noexcept -> bool {
    ...
    iovec iov{inbound_data_.data() + next_recv_valid_index_, TCPBufferSize - next_recv_valid_index_};
    ...
    if (read_size > 0) {
      ...
      if (cmsg->cmsg_level = SOL_SOCKET &&
        ...
        memcpy(&time_kernel, CMSG_DATA(msg), sizeof(time_kernel));
      ...
      logger_.log("%:% %() % read socket:% len:% utime:% ktime:% diff:%\n" __FILE__, __LINE__, __FUNCTION__,
                  Common::getCurrentTimeStr(&time_str_), socket_fd_, next_recv_valid_index_, user_time, kernel_time, (user_time - kernel_time));
